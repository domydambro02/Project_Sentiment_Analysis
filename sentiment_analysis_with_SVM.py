...
# let's print the percentage of each target variable values
...
plt.title('Parole positive comuni nel testo')
...
plt.title('Parole neutrali comuni nel testo')
plt.xlabel('Conteggio')
plt.ylabel('Parola')
plt.show()
...
plt.title('Parole negative comuni nel testo')
plt.xlabel('Conteggio')
plt.ylabel('Parola')
plt.show()

# let's store the modified DataFrame in a new CSV file
data.to_csv('Tweets_restricted.csv', index=False)
...
# Which mean that the data_tweet still have untidy data. There are so many punctuation, constraction, special characters, elongated words, twitter tags and hastag. 
...
plt.title('Word Cloud generale')
plt.show()

# let's store the cleaned up DataFrame in a new CSV file
data_clean.to_csv('Tweets_restricted_cleaned.csv', index=False)

print() 
# let's print the percentage of each target variable values after cleaning
print('Positivi', round(data_clean['sentiment'].value_counts()['positive']/len(data_clean) * 100),'% -->', data_clean['sentiment'].value_counts()['positive'])
print('Neutrali', round(data_clean['sentiment'].value_counts()['neutral']/len(data_clean) * 100),'% -->', data_clean['sentiment'].value_counts()['neutral'])
print('Negativi', round(data_clean['sentiment'].value_counts()['negative']/len(data_clean) * 100),'% -->', data_clean['sentiment'].value_counts()['negative'])
print() 
...
def check_scores(clf, x_train, x_test, y_train, y_test, algorithm_name):
    ...
    ax1.set_title(f'Matrice di confusione di train ({algorithm_name})')
    ax1.set_xlabel('Previsto')
    ax1.set_ylabel('Effettivo')
    ...
    ax2.set_title(f'Matrice di confusione di train di test ({algorithm_name})')
    ax2.set_xlabel('Previsto')
    ax2.set_ylabel('Effettivo')
    ...
    results = pd.DataFrame(columns=['Tweets', 'Ac. Pos.', 'Ac. Neu.', 'Ac. Neg.', 'Er. Pos.', 'Er. Neu.', 'Er. Neg.'])

    # let’s define the tweet intervals to consider
    tweet_intervals = [100, 500, 2000, 5000, 10000, 20000, 27388]
    ...
    for num_tweets in tweet_intervals:
        ...
        new_row = pd.DataFrame({'Tweets': [num_tweets], 'Ac. Pos.': [acc_pos], 'Ac. Neu.': [acc_neu], 'Ac. Neg.': [acc_neg], 'Er. Pos.': [err_pos], 'Er. Neu.': [err_neu], 'Er. Neg.': [err_neg], 'Accuracy': [acc_avg], 'Error': [err_avg]})
        results = pd.concat([results, new_row], ignore_index=True)

        # Configurazione Pandas per visualizzare tutte le colonne e righe
    ...
    # Crea i grafici
    for column, label, color in zip (['Ac. Pos.', 'Ac. Neu.', 'Ac. Neg.', 'Accuracy'], ['Positive', 'Neutral', 'Negative', 'Average'], ['green',  'yellow', 'red', 'black']):
        plt.plot(results['Tweets'], results[column], label=label, color=color)
    plt.xlabel('Numero di tweets')
    plt.ylabel('Accuratezza')
    plt.legend()
    plt.yticks(np.arange(0, 1.05, 0.05))
    plt.show()

    for column, label, color in zip (['Er. Pos.', 'Er. Neu.', 'Er. Neg.', 'Error'], ['Positive', 'Neutral', 'Negative', 'Average'], ['green', 'yellow', 'red', 'black']):
        plt.plot(results['Tweets'], results[column], label=label, color=color)
    plt.xlabel('Numero di tweets')
    plt.ylabel('Errore')
    plt.legend()
...
new_data = pd.read_csv('twitter_ElonMusk_restricted_cleaned.csv')
...
plt.xlabel('Sentiment')
plt.ylabel('Numero di Tweet')
plt.title('Distribuzione dei tweet dopo Sentiment Prection con SVM')
plt.show()
print()
